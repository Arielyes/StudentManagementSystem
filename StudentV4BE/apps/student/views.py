...
def get_students(request):
    ...
    try:
        ...
        # 结果异常，返回
        return JsonResponse({'code': 0, 'msg': "返回结果出现异常：" + str(e)})


def query_students(request):
    '''查询学生信息'''
    # 接收传递过来的查询条件---axios，默认是json---字典类型('inputstr')---data['inputstr']
    data = json.loads(request.body.decode('utf-8'))
    try:
        # 使用ORM获取满足条件的学生的信息，并把对象转为字典格式
        obj_students = Student.objects.filter(Q(sno__icontains=data['inputstr']) | Q(name__icontains=data['inputstr']) |
            ...
            address__icontains=data['inputstr'])
                                              ).values()
        # 把结果转为list
        students = list(obj_students)
        # 返回成功
...
def add_student(request):
    ...
    try:
        ...
        return JsonResponse({'code': 0, 'msg': "添加学生到数据库失败,具体原因：" + str(e)})
...
def update_student(request):
    ...
    try:
        ...
        return JsonResponse({'code': 0, 'msg': "修改学生到数据库失败,具体原因：" + str(e)})
...
def delete_students(request):
    ...
    try:
        ...
        for student in data['students']:
            student = Student.objects.get(sno=student['sno'])
            # 删除
            student.delete()
        # 使用ORM获取所有学生的信息
        obj_students = Student.objects.all().values()
        # 把外层容器转为list
        allStudents = list(obj_students)
        # 返回成功
        return JsonResponse({'code': 1, 'data': allStudents})
    except Exception as e:
...
def upload(request):
    ...
    try:
        ...
        return JsonResponse({'code': 0, 'msg': "图片写入失败" + str(e)})


# 从excel批量导入学生信息
def import_students_excel(request):
    # 1.读取接收Excel文件存储到media文件夹
    rev_file = request.FILES.get('Excel')
    # 判断是否有文件
    if not rev_file:
        return JsonResponse({'code': 0, 'msg': "Excel文件不存在"})
    # if (os.path.splitext(rev_file.name)[1] != ".xlsx") or (os.path.splitext(rev_file.name)[1] != ".xls"):
    #     return JsonResponse({'code': 0, 'msg': "文件不是xlsx或xls文件"+os.path.splitext(rev_file.name)[1]})
    # 获得一个唯一的名字：uuid+hash
    new_name = get_random_str()
    # 准备写入的url
    file_path = os.path.join(settings.MEDIA_ROOT, new_name + os.path.splitext(rev_file.name)[1])
    ...
    try:
        ...
        return JsonResponse({'code': 0, 'msg': "Excel文件导入失败" + str(e)})
    # 2.读取存储在media文件夹中的数据
    ex_students = read_excel_dict(file_path)

    # 3.把读取的数据存储到数据库
    # 定义变量：success:error:errors
    success = 0
    error = 0
    error_snos = []
    # 开始遍历
    for student in ex_students:
        try:
            obj_student = Student(sno=student['sno'], name=student['name'], gender=student['gender'],
                                  birthday=student['birthday'], mobile=student['mobile'],
            ...
            error += 1
            error_snos.append(student['sno'])

    # 4.返回——导入信息（成功--5条，失败--4条），所有学生
    obj_students = Student.objects.all().values()
    students = list(obj_students)
